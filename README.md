...
This is a frameless, desktop Pomodoro timer with a custom title bar, three session modes (pomodoro / short break / long break), and automatic cycling through the standard 25-5-15 Pomodoro Technique — with a long break every 4th work session. The pomodoro count persists across restarts.
...
**Persistent pomodoro count** — `window.pomodoroStore.getCount()` / `.setCount()` read and write a value held in an `electron-store` instance living in the main process. `getCount()` uses `ipcRenderer.invoke` / `ipcMain.handle` (request/response) rather than fire-and-forget `send`, since the renderer needs the saved value back on startup. The count is written to a small JSON file in Electron's per-OS user data directory, so it survives closing and reopening the app.
